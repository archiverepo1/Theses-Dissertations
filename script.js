...
const SOURCES = [
  { name: "University of Cape Town (UCT)", country: "South Africa",
    oai: "https://open.uct.ac.za/server/oai/request", type:"dspace" },
  { name: "Stellenbosch University (SUNScholar)", country: "South Africa",
    oai: "https://scholar.sun.ac.za/server/oai/request", type:"dspace" },
  { name: "University of Pretoria (UPSpace)", country:"South Africa",
    oai:"https://repository.up.ac.za/server/oai/request", type:"dspace" },
  { name: "Wits (WIReDSpace)", country:"South Africa",
    oai:"https://wiredspace.wits.ac.za/server/oai/request", type:"dspace" },
  { name: "North-West University (NWU)", country:"South Africa",
    oai:"https://repository.nwu.ac.za/server/oai/request", type:"dspace" },
  { name: "University of KwaZulu-Natal (UKZN)", country:"South Africa",
    oai:"https://researchspace.ukzn.ac.za/server/oai/request", type:"dspace" },
  { name: "University of the Free State (UFS)", country:"South Africa",
    oai:"https://scholar.ufs.ac.za/server/oai/request", type:"dspace" },
  { name: "University of the Western Cape (UWC)", country:"South Africa",
    oai:"https://uwcscholar.uwc.ac.za/server/oai/request", type:"dspace" },
  { name: "University of South Africa (UNISA)", country:"South Africa",
    oai:"https://uir.unisa.ac.za/server/oai/request", type:"dspace" },
  { name: "Central University of Technology (CUT)", country:"South Africa",
    oai:"https://cutscholar.cut.ac.za/server/oai/request", type:"dspace" },
  { name: "Cape Peninsula University of Technology (CPUT)", country:"South Africa",
    oai:"https://etd.cput.ac.za/server/oai/request", type:"dspace" }
];
...
async function harvestFirst(url, typeFilter) {
  ...
  try {
    ...
    recs.forEach(r => {
      ...
      const link   = httpLink || (doi ? `https://doi.org/${doi}` : "");
...
function initHeroBg() {
  ...
  let w,h,pts;
  function resize(){ w=canvas.width=window.innerWidth; h=canvas.height=260; pts=Array.from({length:50},()=>({x:Math.random()*w, y:Math.random()*h, vx:(Math.random()-0.5)*0.5, vy:(Math.random()-0.5)*0.5}));}
  resize();
...
async function populate() {
  ...
  SOURCES.forEach(src=>{
    ...
    CACHE.set(src.name, { online:false, items:null });
  });
  ...
  for (const src of SOURCES) {
    ...
    const meta = document.createElement("div"); meta.className = "inst-meta";
    const h3 = document.createElement("h3"); h3.className="inst-name"; h3.textContent=src.name;
    const p = document.createElement("p"); p.className="inst-country"; p.textContent=src.country;
    meta.append(h3, p);
    ...
    if (SELECTED_INST.size===0 || SELECTED_INST.has(src.name)) {
      ...
      if (src.oai) {
        ...
        if (cache.online && !cache.items) {
          ...
          items.forEach(it => { it.institution = src.name; it.country = src.country; });
          cache.items = items;
          CACHE.set(src.name, cache);
        }
